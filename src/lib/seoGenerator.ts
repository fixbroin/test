...
export function getSpinnedLocalContent(data: SeoPlaceholderData & { templateIdx?: number }): string {
  ...
  const paragraph1List = [
    `Need professional ${targetLabel} services in ${locationLabel}? Wecanfix connects customers with trusted local experts who provide reliable service solutions for homes, apartments, offices, retail shops, and commercial properties throughout ${locationLabel} and neighbouring areas like ${formattedAreas}.`,
    `Looking for certified ${targetLabel} specialists near you in ${locationLabel}? Wecanfix brings you background-verified trade experts equipped with advanced tools to handle home maintenance, retail support, and corporate installations around ${locationLabel} and neighbouring localities like ${formattedAreas}.`,
    `If you are searching for high-quality ${targetLabel} in ${locationLabel}, our digital booking platform offers instant scheduling. Wecanfix connects you with skilled pros serving residential complexes, offices, and retail properties in ${locationLabel} as well as adjacent sectors including ${formattedAreas}.`,
    `Get verified doorstep assistance for ${targetLabel} in ${locationLabel}. Wecanfix bridges the gap between home-owners and certified service professionals, delivering premium quality solutions across the entire ${locationLabel} region and surrounding areas like ${formattedAreas}.`
  ];
  ...
  const paragraph4List = [
    `If you are searching for trusted ${targetLabel} near ${locationLabel}, ${cityName}, Wecanfix helps you connect with experienced professionals for quality service and dependable support.`,
    ...
    `Get connected to the best ${targetLabel} near you in ${locationLabel} today. Experience standard-setting doorstep maintenance with Wecanfix.`,
    `Searching for affordable ${targetLabel} near you in ${locationLabel}? Look no further. Wecanfix guarantees high-quality repairs with maximum convenience.`
  ];
...
export function generateFreeCitySeoData(cityName: string, categoryNames: string[], nearbyAreas?: Array<{ id: string; name: string; slug: string }>) {
  ...
  const descTemplates = [
    `Looking for trusted home services in ${cityName} near you? Wecanfix connects you with certified experts for ${catsText}${localCoverageText}. Book online with upfront pricing!`,
    ...
    `Wecanfix offers high-quality home repair in ${cityName} near you. Book verified experts for ${catsText}${localCoverageText} with upfront rates and guaranteed support.`
  ];
  ...
  const generic = [
    ...
    `wecanfix ${cityName}`, `booking app ${cityName}`, `professional service ${cityName}`,
...
export function generateFreeAreaSeoData(cityName: string, areaName: string, categoryNames: string[], nearbyAreas?: Array<{ id: string; name: string; slug: string }>) {
  ...
  const descTemplates = [
    `Looking for trusted home services in ${areaName}, ${cityName} near you? Wecanfix connects you with certified experts for ${catsText}${localCoverageText}. Book online with upfront pricing!`,
    ...
    `Wecanfix offers high-quality home repair in ${areaName} near you. Book verified experts for ${catsText}${localCoverageText} with upfront rates and guaranteed support.`
  ];
  ...
  const generic = [
    ...
    `wecanfix ${areaName}`, `booking app ${areaName}`, `professional service ${areaName}`,
...
export function generateFreeCityCategorySeoData(cityName: string, categoryName: string, serviceNames: string[], nearbyAreas?: Array<{ id: string; name: string; slug: string }>) {
  ...
  const titleTemplates = [
    ...
    `Doorstep ${categoryName} Services in ${cityName} Near You | Wecanfix`
  ];

  const descTemplates = [
    `Looking for trusted ${categoryName.toLowerCase()} services in ${cityName} near you? Wecanfix connects you with certified experts for ${servicesText}${localCoverageText}. Book online with upfront pricing!`,
    ...
    `Wecanfix offers top-quality ${categoryName.toLowerCase()} services in ${cityName} near you. Book verified specialists for ${servicesText}${localCoverageText} with upfront rates.`
  ];
  ...
  const generic = [
    ...
    `wecanfix ${cleanCat} ${cityName}`, `booking app ${cleanCat} ${cityName}`, `professional ${cleanCat} service ${cityName}`,
  ...
  const faqsList = [
    {
      ...
      answer: `Yes, Wecanfix provides comprehensive, top-rated ${categoryName.toLowerCase()} services throughout ${cityName} and surrounding localities.`
    },
    {
...
export function generateFreeAreaCategorySeoData(cityName: string, areaName: string, categoryName: string, serviceNames: string[], nearbyAreas?: Array<{ id: string; name: string; slug: string }>) {
  ...
  const titleTemplates = [
    ...
    `Doorstep ${categoryName} Services in ${areaName} Near You | Wecanfix`
  ];

  const descTemplates = [
    `Looking for trusted ${categoryName.toLowerCase()} services in ${areaName}, ${cityName} near you? Wecanfix connects you with certified experts for ${servicesText}. Book online with upfront pricing!`,
    ...
    `Wecanfix offers top-quality ${categoryName.toLowerCase()} services in ${areaName} near you. Book verified specialists for ${servicesText} covering ${nearbyList.slice(0, 3).join(', ')}.`
  ];
  ...
  const generic = [
    ...
    `wecanfix ${cleanCat} ${areaName}`, `booking app ${cleanCat} ${areaName}`, `professional ${cleanCat} service ${areaName}`,
  ...
  const faqsList = [
    {
      ...
      answer: `Yes, Wecanfix provides comprehensive, top-rated ${categoryName.toLowerCase()} services throughout ${areaName} and surrounding areas like ${formattedAreas}.`
    },
    {
...
export function generateFreeAreaServiceSeoData(cityName: string, areaName: string, categoryName: string, serviceName: string, nearbyAreas?: Array<{ id: string; name: string; slug: string }>) {
  ...
  const titleTemplates = [
    ...
    `Doorstep ${serviceName} Services in ${areaName} Near You | Wecanfix`
  ];

  const descTemplates = [
    `Looking for trusted ${serviceName.toLowerCase()} in ${areaName}, ${cityName} near you? Wecanfix connects you with certified experts. Book online with upfront pricing!`,
    ...
    `Wecanfix offers top-quality ${serviceName.toLowerCase()} services in ${areaName} near you. Book verified specialists covering ${nearbyList.slice(0, 3).join(', ')}.`
  ];
  ...
  const generic = [
    ...
    `wecanfix ${cleanSrv} ${areaName}`, `booking app ${cleanSrv} ${areaName}`, `professional ${cleanSrv} service ${areaName}`,
  ...
  const faqsList = [
    {
      ...
      answer: `Yes, Wecanfix provides comprehensive, top-rated ${serviceName.toLowerCase()} throughout ${areaName} and surrounding areas like ${formattedAreas}.`
    },
    {
